...
def main(project_root=None, projects_root=None):
    ...
    t0 = time.time()
    if not paths['pairwise_dir'].exists():
        print(f"ERROR: Pairwise70 data directory not found: {paths['pairwise_dir']}")
        return None
    study_reviews, review_studies, review_names = load_all_studies(paths['pairwise_dir'])
    if not study_reviews or not review_studies:
        print(f"ERROR: No review/study data found in {paths['pairwise_dir']}")
        return None
