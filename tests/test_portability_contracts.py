...
class PortabilityContracts(unittest.TestCase):
    ...
    def test_generate_figures_uses_repo_relative_base_dir(self) -> None:
        text = (REPO_ROOT / "generate_figures.py").read_text(encoding="utf-8")

        self.assertIn("Path(__file__).resolve().parent", text)
        self.assertNotIn(r"C:\OverlapDetector", text)
